...
data <- read_csv("/Users/natan/Dev/education_research/descriptive_analysis/mathpass_by_black_hispanic.csv", col_types = cols(`...1` = col_skip()))
...
data_long <- data %>%
  ...
  group_by(state, bh_level) %>%
  arrange(state, bh_level, Year) %>%
  mutate(First_Non_NA = first(ELA_Pass_Rate, order_by = Year)) %>%
  ungroup()

# Before mutating the bh_level column
data_long$Original_Group <- data_long$bh_level
...
data_long <- data_long %>%
  mutate(bh_level = case_when(
    bh_level == "mid" ~ paste0("   ", state_abbreviations_vector[state], "      Mid"),
    bh_level == "high" ~ "High",
    bh_level == "low" ~ "Low",
    TRUE ~ as.character(bh_level)
  ))

# Explicitly order the Groupså
data_long$bh_level <- factor(data_long$bh_level, 
                          levels = c("High",
...
p <- ggplot(data_long, aes(x = Delta_Change, y = bh_level)) +
  ...
  labs(x = "Math Pass Rate Change from Baseline", 
       y = "State", 
       color = "Black & Hispanic Population level") +
...
ggsave(filename = "/Users/natan/Dev/education_research/figures/mathpass_by_black_hispanic.png", bg = "white", plot = p, width = 5, height = 8)
...
# Read the sample data
data <- read_csv("/Users/natan/Dev/education_research/descriptive_analysis/elapass_by_black_hispanic.csv", col_types = cols(`...1` = col_skip()))
colnames(data) <- c("state", "bh_level", "2015", "2016", "2017", "2018", "2019", "2021" )
...
ggsave(filename = "/Users/natan/Dev/education_research/figures/elapass_by_black_hispanic.png", bg = "white", plot = p, width = 5, height = 8)
...
ggsave(filename = "/Users/natan/Dev/education_research/figures/dropout_by_black_hispanic.png", bg = "white", plot = p, width = 5, height = 8)
